...
.addRow(
  ...
  .addPanel(
    ...
    .addTarget(
      prometheus.target(
        // TODO
        expr='sum by (cluster, dc, rack, instance, pool_name) (' + prefix + '_thread_pools_pending_tasks{pool_name=~"#TODO#read_stage|request_response_stage|internal_response_stage|compaction_executor", cluster="$cluster", dc=~"$dc", rack=~"$rack", instance=~"$node"})',
  ...
  .addPanel(
    ...
    .addTarget(
      prometheus.target(
      // TODO @Alain
        expr='sum by (cluster, dc, rack, instance, pool_name) (' + prefix + '_thread_pools_total_blocked_tasks_total{pool_name=~"#TODO#read_stage|request_response_stage|internal_response_stage|compaction_executor", cluster="$cluster", dc=~"$dc", rack=~"$rack", instance=~"$node"})',
  ...
  .addPanel(
    ...
    .addTarget(
      prometheus.target(
        // TODO @Alain
        expr='sum by (cluster, dc, rack, instance, message_type) (rate(' + prefix + '_dropped_message_dropped_total{message_type=~"#TODO#read|request_response|range_slice|paged_range",cluster="$cluster", dc=~"$dc", rack=~"$rack", instance=~"$node"}[1m]))',
...
// Cassandra
// Section with:
//    - pending/dropped/blocked flushes
//    - memtable sizes
...
// Clock drift (here and in overview!)
